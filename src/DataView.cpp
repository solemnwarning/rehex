...
REHex::FlatDocumentView::FlatDocumentView(const SharedDocumentPointer &document):
	document(document)
{
	this->document.auto_cleanup_bind(DATA_ERASING,           &REHex::FlatDocumentView::OnDataEvent, this);
	this->document.auto_cleanup_bind(DATA_ERASE_ABORTED,     &REHex::FlatDocumentView::OnDataEvent, this);
	this->document.auto_cleanup_bind(DATA_ERASE,             &REHex::FlatDocumentView::OnDataEvent, this);
	
	this->document.auto_cleanup_bind(DATA_INSERTING,         &REHex::FlatDocumentView::OnDataEvent, this);
	this->document.auto_cleanup_bind(DATA_INSERT_ABORTED,    &REHex::FlatDocumentView::OnDataEvent, this);
	this->document.auto_cleanup_bind(DATA_INSERT,            &REHex::FlatDocumentView::OnDataEvent, this);
	
	this->document.auto_cleanup_bind(DATA_OVERWRITING,       &REHex::FlatDocumentView::OnDataEvent, this);
	this->document.auto_cleanup_bind(DATA_OVERWRITE_ABORTED, &REHex::FlatDocumentView::OnDataEvent, this);
...
REHex::LinearVirtualDocumentView::LinearVirtualDocumentView(const SharedDocumentPointer &document):
	document(document),
	total_view_length(0),
	pending_erase_offset(-1),
	pending_erase_length(-1)
{
	...
	//this->document.auto_cleanup_bind(DATA_INSERT,            &REHex::LinearVirtualDocumentView::OnDataInsert,           this);
	
	this->document.auto_cleanup_bind(DATA_OVERWRITING,       &REHex::LinearVirtualDocumentView::OnDataOverwriteEvent, this);
	this->document.auto_cleanup_bind(DATA_OVERWRITE_ABORTED, &REHex::LinearVirtualDocumentView::OnDataOverwriteEvent, this);
	this->document.auto_cleanup_bind(DATA_OVERWRITE,         &REHex::LinearVirtualDocumentView::OnDataOverwriteEvent, this);
	
	load_segments();
}

void REHex::LinearVirtualDocumentView::load_segments()
{
	virt_to_real_segs = document->get_virt_to_real_segs();
...
std::vector<bool> REHex::LinearVirtualDocumentView::read_bits(BitOffset view_offset, size_t max_length) const
{
	...
	for(auto it = view_to_real_segs.get_range(view_offset.byte()); it != view_to_real_segs.end(); ++it)
	{
...
REHex::ByteRangeSet REHex::LinearVirtualDocumentView::find_event_ranges(const OffsetLengthEvent &event)
{
...
void REHex::LinearVirtualDocumentView::OnDataErasing(OffsetLengthEvent &event)
{
	...
	ByteRangeSet ranges = find_event_ranges(event);
	lock_guard.unlock();
	
	process_event_for_ranges_desc(DATA_ERASING, ranges);
...
void REHex::LinearVirtualDocumentView::OnDataEraseAborted(OffsetLengthEvent &event)
{
	...
	process_event_for_ranges_desc(DATA_ERASE_ABORTED, ranges);
...
void REHex::LinearVirtualDocumentView::OnDataOverwriteEvent(OffsetLengthEvent &event)
{
	std::unique_lock<shared_mutex> lock_guard(mutex);
	ByteRangeSet ranges = find_event_ranges(event);
	lock_guard.unlock();
