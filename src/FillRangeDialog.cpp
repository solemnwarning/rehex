...
void REHex::FillRangeDialog::OnOK(wxCommandEvent &event)
{
	...
	try {
		...
		{
			selection_off = range_from->GetNumericValue<off_t>(0, doc_length);
		}
		else{
			selection_off = range_from->GetNumericValue<off_t>(0, (doc_length - 1));
		}
	}
	...
	if(range_to_enable->GetValue())
	{
		try {
			if(insert_mode_selected)
			{
				off_t selection_to = range_to->GetNumericValue<off_t>(selection_off);
				selection_length = (selection_to - selection_off) + 1;
			}
			else{
				off_t selection_to = range_to->GetNumericValue<off_t>(selection_off, (doc_length - 1));
				selection_length = (selection_to - selection_off) + 1;
			}
		}
		catch(const NumericTextCtrl::InputError &e)
	...
	else if(range_len_enable->GetValue())
	{
		try {
			if(insert_mode_selected)
			{
				selection_length = range_len->GetNumericValue<off_t>(0);
			}
			else{
				selection_length = range_len->GetNumericValue<off_t>(0, (doc_length - selection_off));
			}
		}
		catch(const NumericTextCtrl::InputError &e)
		{
	...
	*/
	...
	try {
		...
		{
			insert_len = std::min<off_t>(data.size(), (selection_length - data.size()));
			data.insert(data.end(), data.begin(), std::next(data.begin(), insert_len));
		}
