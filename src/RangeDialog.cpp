...
void REHex::RangeDialog::OnOK(wxCommandEvent &event)
{
	...
	try {
		virt_offset = range_from->GetNumericValue<BitOffset>(BitOffset::MIN, BitOffset::MAX, BitOffset::ZERO, 0, &virt_offset_bit);
	}
	...
	if(range_to_enable->GetValue())
	{
		...
		bool virt_end_bit;
		try {
			virt_end_incl = range_to->GetNumericValue<BitOffset>(BitOffset::MIN, BitOffset::MAX, BitOffset::ZERO, 0, &virt_end_bit);
		}
		catch(const NumericTextCtrl::InputError &e)
		{
			wxMessageBox(e.what(), "Error", (wxOK | wxICON_ERROR | wxCENTRE), this);
	...
	else if(range_len_enable->GetValue())
	{
		BitOffset length;
		try {
			length = range_len->GetNumericValue<BitOffset>(BitOffset(0, 1));
		}
		catch(const NumericTextCtrl::InputError &e)
		{
			wxMessageBox(e.what(), "Error", (wxOK | wxICON_ERROR | wxCENTRE), this);
