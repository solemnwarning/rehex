	...
	mappings_changed_buffer(this, EV_MAPPINGS_CHANGED)
{
	...
	std::string filename = this->buffer->get_filename().GetFullPath().ToStdString();
	if(filename.empty())
	{
		title  = "Untitled";
	}
...
void REHex::Document::reload()
{
	...
	FileName filename = buffer->get_filename();
...
void REHex::Document::save()
{
	...
	save_metadata_for(buffer->get_filename().GetFullPath().ToStdString());
...
bool REHex::Document::is_range_dirty(BitOffset offset, BitOffset length) const
{
	off_t next = offset.byte();
	off_t end = (offset + length).byte_round_up();
	
	for(
		ByteRangeMap<unsigned int>::const_iterator ds;
		next < end && (ds = data_seq.get_range_in(next, (end - next))) != data_seq.end();
		next = ds->first.offset + ds->first.length)
	{
		if(ds->second != saved_seq)
		{
			return true;
		}
	}
