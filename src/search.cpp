...
void REHex::Search::OnTimer(wxTimerEvent &event)
{
	if(m_find_multiple)
	{
		...
		if(num_results >= MAX_RESULTS && task.finished() && next_window_start < search_end)
		{
			...
			wxMessageBox("Too many occurrences found. Search stopped.", "Warning",
