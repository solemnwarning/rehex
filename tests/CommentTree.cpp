...
static void build_model_values(std::vector<std::string> &values, const std::string &prefix, wxDataViewItem node, const REHex::CommentTreeModel *model)
{
	...
	is_int(items.GetCount(), n_items, "CommentTreeModel::GetChildren(%s) returns array size", prefix.c_str());
	...
	for(unsigned int i = 0; i < items.GetCount(); ++i)
	{
		...
		ok((parent.GetID() == node.GetID()), "CommentTreeModel::GetParent(%s) returns parent node", item_string.c_str());
...
static void check_values(const REHex::CommentTreeModel *model, ...)
{
	...
	ok((got_values == expect_values), "CommentTreeModel returns correct values");
}
...
static void refresh_check_notifications(REHex::CommentTreeModel *model, ...)
{
	...
	if(!ok((notifier->events == expect), "refresh_comments() generated expected notifications"))
	{
		diag("Expected:");
		for(size_t i = 0; i < expect.size(); ++i)
		{
			diag("%d: %s", (int)(i), expect[i].c_str());
		}
		
		diag("Got:");
		for(size_t i = 0; i < notifier->events.size(); ++i)
		{
			diag("%d: %s", (int)(i), notifier->events[i].c_str());
		}
	}
...
int main(int argc, char **argv)
{
	...
	{
		diag("Testing with a single comment");
	...
	{
		diag("Testing with multiple comments");
	...
	{
		diag("Testing with a heirarchy of comments");
	...
	{
		diag("Testing erasing a comment at root level with no chidren");
	...
	{
		diag("Testing erasing a comment at root level with chidren");
	...
	{
		diag("Testing erasing a nested comment with no chidren");
	...
	{
		diag("Testing erasing a nested comment with chidren");
	...
	{
		diag("Testing adding a comment at the root");
	...
	{
		diag("Testing adding a nested comment");
